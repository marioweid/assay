...
## 2026-09-22 — E3 runnable deployment documentation implemented

[OUTCOME] Added Linux-first and PowerShell-parity quickstarts, deployment/recovery guidance, a concise release-first README with the exact published Compose content, and current-checkout SDK quickstart artifacts. The guides distinguish admin, project ingest, database, judge, and encryption credentials; keep the unpublished image explicitly unreleased; cover trace-only and optional evaluation paths; and document snapshots, destructive volume/dataset operations, backups, restore-to-new-database, migrations, and rollback limits.
[OUTCOME] `tests/acceptance/docs-smoke.sh` now detects README/Compose drift, checks local Markdown links and Linux credential generation, rejects non-E1 endpoints, and emits plus queries a real quickstart SDK trace through the disposable fixture stack. `tests/acceptance/run.sh` invokes it before SDK lifecycle acceptance. A focused review found workspace-output collision could create orphaned resources; the bootstrap now reserves its private output before API mutations and deletes the new project if setup later fails.
[VERIFY] Static docs smoke passed with a local OpenSSL-compatible test shim because this host lacks `openssl`; `bash -n`, Docker Compose validation for source and synthetic published environments, Python syntax compilation inside the existing `assay-chat` container, and `git diff --check` pass. Full E1 docs workflow could not run locally because the SDK virtualenv hits NixOS's dynamic-linker stub and Playwright Chromium is likewise unavailable. PowerShell requires a Windows runner and is not claimed verified.

## 2026-09-22 — Web CI SDK-tool provisioning fixed

[OUTCOME] Web CI failed from M7D6 onward because embedded acceptance invokes `uv`, while `web.yml` only installed Go and Node. It now reuses the pinned `astral-sh/setup-uv` action and version from the Python CI workflow.
[VERIFY] The failing GitHub run and every run since M7D6 stop at embedded acceptance; the prior E1 run passed. Local E1 browser acceptance passes through all eight tests, then correctly reaches the SDK/docs `uv` path but cannot execute its dynamically linked virtualenv on NixOS. Workflow diff check and pinned-action presence check pass. GitHub-run confirmation remains pending.

## 2026-09-23 — Persistent Docker, dummy application, and live skill-agent validation

[OUTCOME] Built clean `main` checkout `aa30031` into a separate persistent Compose project `assay-acceptance-persistent`, leaving the original port-8080 stack untouched. Assay is at `http://127.0.0.1:18080`; an interactive deterministic Python SDK dummy is at `http://127.0.0.1:18090`. Workspace, private credentials, report, runnable scripts, source snapshot, logs, and screenshots live at `C:/Users/mario/sources/assay-local-validation/`. All four services remain healthy with restart policies; no paid model calls were made.
[VERIFY] 323 Linux Docker Go race tests pass with no test skips; 158 frontend tests plus lint/typecheck/build and dependency audit pass; 239 Python tests plus ruff/format/ty pass (one opt-in real-judge test skipped); eight existing Playwright/axe tests pass. The dummy exercises real HTTP generation, nested SDK capture, automatic good/bad scores, both evaluation modes, a +0.5 paired improvement, and an intentional HTTP-400 failed run. A separate pi agent read the checked-in Assay skill and completed 37 live CLI outcomes: trace inspection, retained-evidence import/409 duplicate, baseline scores 0/gate exit 1, generated scores 1/gate exit 0, +1 comparisons, failed exports, and metrics. Its project and all evidence are retained.
[VERIFY] Linux Docker docs smoke passes. Portable checksum-verified PowerShell 7.6.6 on Windows passes documented credential generation, readiness, bootstrap, real SDK trace/CLI lookup, and dataset import. Optional docs real-judge evaluation and GitHub evidence are not claimed. Native Git Bash docs smoke initially failed on OpenSSL CRLF; one native Windows Go test failed Docker-provider discovery; the corresponding Linux Docker checks passed without product changes.
[VERIFY] Force-recreated all four services without changing the named volume: 6 projects, 6 applications, 6 datasets, 8 items, 15 traces, 36 spans, 7 runs, and 39 scores were unchanged, and the retained agent candidate still passed both CLI gates. A custom-format backup restored into separate retained database `assay_restore_check` with identical counts. The live service continues to use database `assay`.
[FINDINGS] Additional browser scans of 12 populated pages at 360/1440px in light/dark themes loaded all 48 without console/page errors. Ten scans expose three accessibility defects: unnamed action header in `web/src/features/projects/api-keys-panel.tsx:232`; unfocusable mobile scroll containers in `web/src/features/runs/run-detail.tsx:205` and `web/src/features/metrics/metrics-page.tsx:83`. Screenshots and axe output are preserved; E4 accessibility approval is not claimed. Skill text incorrectly says scorer configuration is thresholds-only. The original `assay-chat-1` was already restart-looping and was not modified.
[NEXT] User inspects `assay-local-validation/REPORT.md`, its dummy app, and retained agent project. Address accessibility/skill guidance in subsequent work; no product-source changes or commits were made during this validation.

## 2026-09-24 — Populated-page accessibility and Assay skill corrected

[OUTCOME] Named the API key action header and provided labelled, keyboard-focusable scroll regions with visible focus styling for run aggregates and metrics. Updated the checked-in Assay skill to distinguish low-quality traces from execution errors, document UTF-8 reference file attachment, and reflect the current scorer CLI options. Rebuilt only the persistent validation `assayd` image (`assay-local-validation:a11y-fix`), preserving its database and existing projects; the private Compose image setting now points at the fixed image.
[VERIFY] The new E1 populated-page test failed on the unnamed header before the fix, then passed after it; it creates a dataset/run in its disposable fixture project, scans all three loaded pages with axe at 360px, and keyboard-scrolls the two wide tables. Full E1 browser suite: 9 passed. Broader populated-page scan: 48/48 pages without axe violations or browser errors (previously 10 affected scans). Frontend 158 Vitest tests, lint, format, typecheck, and production build pass. The checked-in skill was read and used directly against live Docker using current CLI: filtered two low-quality traces, inspected retained rationale, listed failed scores, confirmed baseline gate exit 1 / candidate gate exit 0, +1 paired comparison, and queried metrics. No project mutations during direct skill check; original agent project's evidence retained. Detailed before/after results: `C:/Users/mario/sources/assay-local-validation/FOLLOWUP-2026-09-24.md`.
[NEXT] E4 cross-browser/visual/performance approval and GHCR/GitHub-run evidence are still pending; Chromium/axe checks are not a Safari sign-off.

## 2026-09-25 — Persistent local chat and general questions

[OUTCOME] Reused the existing `python-qa-example` project/application, repaired the chat's mismatched admin token by recreating its container, and added a private, persistent Ollama overlay: Qwen3:4b for general/domain responses, Mistral:7b for project-scoped groundedness judging. Every turn has a trace; only explicit Assay questions are eligible for contextual scoring (including the CLI path). The invalid root hosted key was not used; model server has no published port. All root containers now honor the checked-in loopback-only port bindings. Independent persistent validation on port 18080 was left intact. Private root database backup and operational evidence are in `../assay-local-validation/CHAT-2026-09-25.md`.
[VERIFY] 15 example tests, Ruff check/format, ty, Compose validation and diff check pass; browser/HTTP general answer persisted an unscored trace, an Assay answer got a successful local Mistral score, and the real CLI general answer ran after a fresh image rebuild. Containers and volumes survived a Docker daemon restart; root and validation stacks returned healthy. Independent review found CLI mis-scoring and optional paid-judge instructions; both were fixed, and focused recheck passed.
[LIMIT] A repeat Assay question answered from context and persisted a trace, but its local Mistral judge failed strict JSON verification twice; one prior Assay question had scored 1. Local groundedness scoring is intermittent, not guaranteed. The scorer's generic error does not preserve the invalid field/raw response; do not claim the exact malformed field.
[OUTCOME] Chat implementation committed as `8ccb3f6` on `feat/local-general-chat`; the session explorer design remains planning-only. Previous accessibility PR #19 CI fails because container smoke expects `rg` missing on its runner. Eight active temporary ingest keys remain on the reused project; seven predate the current chat container and were not revoked without explicit permission.

## 2026-09-25 — Planned cross-trace sessions and modern UI

[DECISION] The user requested Sessions for explicitly `session.id`-tagged traces while the existing Traces view remains complete. The example chat retains its session through refresh/reopen until New Chat, with backend-issued IDs and a separate anonymous browser identity. Midnight slate/teal is the dark-first visual direction, with existing light/system choices retained. This expands the earlier M7 scope that excluded cross-trace sessions; implementation has not begun.
[PLAN] `docs/plans/2026-09-25-session-explorer-and-ui.md` phases: approve UX mockups, project-scoped indexed session queries and generated clients, async-safe SDK context, secure backend-issued cookie/rehydrated chat turns, session/trace/Gantt workbench and shared chat styling, isolated browser/SDK/API acceptance. OTel `session.id` groups traces, `gen_ai.conversation.id` identifies this demo's chat thread, and `enduser.pseudo.id` is deliberately not authenticated `user.id`. Browser history retrieval needs a signed bearer capability distinct from the public session ID; the plan documents expiry/revocation limits. No data or services were modified for planning.

## 2026-09-25 — Standalone visual mockup for user review

[OUTCOME] Created a single-file, dependency-free midnight-slate/teal Sessions preview at `mokeup/index.html`, plus desktop/mobile screenshots and a brief README. It shows a session list, aligned user/assistant turns, selectable trace inspection, context evidence and a small timing Gantt with sample data only; no live endpoints or migrations were touched.
